...
GrovePi.prototype.toString = function() {
	return "GrovePi";
}

GrovePi.prototype.input = function(channel, callback) {
	var name = this.name;
	$.get(this.url + "/input/" + channel, function(data) {
		callback(name, channel, data);
	});
...
GrovePi.prototype.output = function(channel, value, callback) {
	var name = this.name;
	$.post(this.url + "/output/" + channel + "/" + value, function(data) {
		callback(name, channel, data);
	});
...
GrovePi.prototype.readAll = function(callback) {
	var name = this.name;
	$.get(this.url+ "/*", function(data) {
		callback(name, data);
	});
...
GrovePi.prototype.refreshUI = function() {
	var port = this;
	var element = this.element;
	if ((element != undefined) && (element.header == undefined)) {
		element.header = $("<h3>" + this + "</h3>");
	...
	if ((element != undefined) && (element.table == undefined)) {
		...
		var line = $("<tr>");
		line.append($("<td><b>Digital Outputs</b></td>"))
		for (var i = 2; i<=8; i++) {
			var cell = $("<td>");
			var button = webiopi().createButton(this.name + "_output_" + i, i, function() {
				if ($("#" + port.name + "_output_" + $(this).attr("channel")).attr("class") == "LOW") {
